...
test.describe('broadsheet column alignment', () => {
  test('wordmark and footer content left-align with the opening thesis at 1280x800', async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 800 });
    await page.goto('/');

    const openingBox = await page.locator('.opening-eyebrow').boundingBox();
