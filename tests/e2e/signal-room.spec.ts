...
test('signal room: index lists every episode newest-first', async ({ page }) => {
  ...
  await expect(links.first()).toContainText('the honest gray');
...
test('signal room: the index shows a date for each episode', async ({ page }) => {
  await page.goto('/signal-room');
  await expect(page.locator('main')).toContainText('August 17, 2026');
...
test('signal room: episode 12 renders prose and has a served OG card', async ({ page }) => {
  await page.goto('/signal-room/the-honest-gray');

  await expect(page.locator('main h1')).toHaveText('the honest gray');
  await expect(page.locator('main')).toContainText(
    'a green light is a receipt, not a mood. when the receipt expires, gray is the only honest color.',
  );
  await expect(page.locator('main')).toContainText('August 17, 2026');
  await expect(page.locator('meta[property="og:image"]')).toHaveAttribute(
    'content',
    /\/og\/signal-room\/the-honest-gray\.png$/,
  );

  const img = await page.request.get('/og/signal-room/the-honest-gray.png');
...
test('signal room: episodes appear in the rss feed', async ({ page }) => {
  ...
  expect(body).toContain('signal room 12 · the honest gray');
  ...
  expect(body).toContain('/signal-room/the-honest-gray');
